...
const Profile = () => {
    ...
    useEffect(() => {
        Promise.all(visibleComments.map((comment) => fetch(`http://lbosau.exlb.org:9900/Movie/Info?Mid=${comment.Mid}`).then((response) => response.json())))
        .then((data) => {          
            const movieNames = data.map((movieInfo) => movieInfo.movieinfo.MovieName);
            setMovieNames(movieNames);

            const movieDirector = data.map((movieInfo) => movieInfo.movieinfo.Director);
                        ...
                        <div>
                            ...
                            <div className="inline">
                                ...
                                <div onClick={handleClick} className="banning">
                                    ...
                                    <h4 style={{ marginLeft: "55%", marginTop: "-20px" }}>{banlist.length}</h4>
                                </div>
                                {isDialogOpen && (
                                    <div className="modal">
                                        <div>
                                            ...
                                            {banUser.map(user => (                                               
                                                ...
                                                <div className='inline' key={user.Uid}>
                                                    ...
                                                    />
                                                    <div>
                                                        <h4 className="banlist_user_name">{user.UserName}</h4>
                                                        <h5 className="banlist_email">{user.Email}</h5>
                                                        <div>
                                                            <div className='inline'>
                                                                <div className='banlist_desc'>{user.WishList.length} Wishes ·</div>
                                                                <div className='banlist_desc'>{user.BanList.length} Bans</div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    {<img 
                                                        ...
                                                        onClick={() => {
                                                            ...
                                                            fetch(`http://lbosau.exlb.org:9900/User/Banlist/remove`, {
                                                            method: 'POST',
                                                            headers: {
                                                                'Content-Type': 'application/json'
                                                            },
                                                            ...
                                                            if (response.ok) {
                                                                alert('The user has been removed to the banning list');
                                                                } else {
                                                                throw new Error('Failed to remove banning list');
                                                                }
                                                            }).catch(error => {
                                                                console.log(error);
                                                            });
                    ...
                    {visibleComments.map((comment, index) => (
                        <div key={comment.id}> 
                            ...
                            <div style={{marginLeft: '50px'}}>
                                ...
                                <div className='inline'>
                                    <div>
                                        {<img className="profile-movie_poster" 
                                    ...
                                    <div className='box'>
                                        ...
                                        <>
                                            ...
                                            {[...Array(5 - Math.floor(Math.max(0, scores[index])))].map((_, i) => (
                                            <img
                                                key={i}
                                                className="comment-poster"
                                                src={require("../CommentImage/emptyCircle.png")}
